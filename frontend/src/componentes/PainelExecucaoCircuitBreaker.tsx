...
type ResultadoExecucao = {
  ...
  metricas: {
    ...
    quantidadeFalhasReais: number;
    quantidadeRejeitadasPeloCircuito: number;
    estadoFinalDoCircuito: string;
  };
};

type ChaveVariante = "sem-circuit-breaker" | "com-circuit-breaker";

const VARIANTES: { chave: ChaveVariante; rotulo: string }[] = [
  { chave: "sem-circuit-breaker", rotulo: "Executar sem circuit breaker" },
  { chave: "com-circuit-breaker", rotulo: "Executar com circuit breaker" },
];
...
export function PainelExecucaoCircuitBreaker({ laboratorioId, onResultado }: Props) {
  const [resultados, setResultados] = useState<Record<ChaveVariante, EstadoExecucao>>({
    "sem-circuit-breaker": { status: "ocioso" },
    "com-circuit-breaker": { status: "ocioso" },
  });
      ...
      <div className="flex flex-wrap gap-3">
        {VARIANTES.map(({ chave, rotulo }) => {
          ...
          return (
            ...
            >
              {estado.status === "carregando" ? "Executando 20 chamadas sequenciais…" : rotulo}
            </button>
          );
        })}
      </div>
            ...
            >
              ...
              {estado.status === "carregando" && (
                <p className="text-zinc-500 dark:text-zinc-400">
                  Chamando a dependência instável em sequência…
                </p>
              )}
              {estado.status === "erro" && (
              ...
              {estado.status === "concluido" && (
                <dl className="mt-2 grid grid-cols-2 gap-3 text-zinc-700 sm:grid-cols-4 dark:text-zinc-300">
                  <div>
                    <dt className="text-xs text-zinc-500 dark:text-zinc-400">Falhas reais</dt>
                    <dd className="text-lg font-semibold">
                      {estado.resultado.metricas.quantidadeFalhasReais}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs text-zinc-500 dark:text-zinc-400">Rejeitadas pelo circuito</dt>
                    <dd
                      className={`text-lg font-semibold ${
                        estado.resultado.metricas.quantidadeRejeitadasPeloCircuito > 0
                          ? "text-emerald-600 dark:text-emerald-400"
                          : "text-zinc-500 dark:text-zinc-400"
                      }`}
                    >
                      {estado.resultado.metricas.quantidadeRejeitadasPeloCircuito}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs text-zinc-500 dark:text-zinc-400">Estado final do circuito (REAL)</dt>
                    <dd
                      className={`text-lg font-semibold ${
                        estado.resultado.metricas.estadoFinalDoCircuito === "OPEN"
                          ? "text-orange-600 dark:text-orange-400"
                          : "text-zinc-500 dark:text-zinc-400"
                      }`}
                    >
                      {estado.resultado.metricas.estadoFinalDoCircuito}
                    </dd>
                  </div>
                  <div>
