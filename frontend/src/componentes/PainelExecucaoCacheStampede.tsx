...
type ResultadoExecucao = {
  ...
  metricas: {
    ...
    quantidadeAcessosAoRecursoLentoReal: number;
  };
};

type ChaveVariante = "sem-protecao" | "com-protecao";

const VARIANTES: { chave: ChaveVariante; rotulo: string }[] = [
  { chave: "sem-protecao", rotulo: "Executar sem proteção" },
  { chave: "com-protecao", rotulo: "Executar com proteção (lock no Redis)" },
];
...
export function PainelExecucaoCacheStampede({ laboratorioId, onResultado }: Props) {
  const [resultados, setResultados] = useState<Record<ChaveVariante, EstadoExecucao>>({
    "sem-protecao": { status: "ocioso" },
    "com-protecao": { status: "ocioso" },
  });
            ...
            >
              ...
              {estado.status === "carregando" && (
                <p className="text-zinc-500 dark:text-zinc-400">
                  Chave de cache fria — disputando o recurso lento…
                </p>
              )}
              {estado.status === "erro" && (
              ...
              {estado.status === "concluido" && (
                <dl className="mt-2 grid grid-cols-3 gap-3 text-zinc-700 sm:grid-cols-3 dark:text-zinc-300">
                  <div>
                    <dt className="text-xs text-zinc-500 dark:text-zinc-400">Requisições concorrentes</dt>
                    <dd className="text-lg font-semibold">
                      {estado.resultado.metricas.quantidadeRequisicoesConcorrentes}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs text-zinc-500 dark:text-zinc-400">Acessos ao recurso lento (REAL)</dt>
                    <dd
                      className={`text-lg font-semibold ${
                        estado.resultado.metricas.quantidadeAcessosAoRecursoLentoReal > 1
                          ? "text-red-600 dark:text-red-400"
                          : "text-emerald-600 dark:text-emerald-400"
                      }`}
                    >
                      {estado.resultado.metricas.quantidadeAcessosAoRecursoLentoReal}
                    </dd>
                  </div>
                  <div>
